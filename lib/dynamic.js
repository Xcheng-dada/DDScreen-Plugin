...
// 长图懒加载：分步滚到底再回顶，触发视口外图片开始加载（长opus可达数屏）
...
// 长图图片多、体积大：单图最长 8s，全局兜底 20s（确保底部图片加载完成再截图）
await Promise.race([
...
// B站新版卡片组件（视频/直播/预约等）会按初始容器宽度给 __body 写死宽度，
// 外层卡片被收窄到 632px 后不会重算，右侧文字会溢出被 overflow: hidden 截断；
// 统一改回 flex 自适应，让文本列按收窄后的宽度重新换行
el.querySelectorAll('[class*="bili-dyn-card"][class*="__body"]').forEach(function(b) {
